...
async function exchangeWithEndpoint(url, payload) {
  ...
  if (!res.ok) {
    return { ok: false, endpoint: url, status: res.status, message: await parseError(res) };
  }
...
function normalizeCodeInput(input) {
  ...
  try {
    ...
    const state = url.searchParams.get("state") || url.hash.replace(/^#/, "") || url.searchParams.get("code_verifier");
...
async function authorize(mode) {
  ...
  const url = new URL(`https://${mode === "console" ? PLATFORM_HOST : "claude.ai"}/oauth/authorize`);
...
async function exchange(code, verifier) {
  ...
  return { type: "failed", error: failure?.message || "Token exchange failed.", status: failure?.status, endpoint: failure?.endpoint };
}
...
function billingHeader(body) {
  ...
  const hash = Bun.CryptoHasher.hash("sha256", `${SALT}${sample}${VERSION}`, "hex").slice(0, 3);
  const entry = process.env[ENTRY]?.trim() || "cli";
  const cch = Array.from(crypto.getRandomValues(new Uint8Array(3))).map(b => b.toString(16).padStart(2, "0")).join("").slice(0, 5);
...
function transformBody(body) {
  ...
  if (Array.isArray(json.system)) {
    json.system = json.system.map((item) => {
      if (!item || typeof item !== "object" || item.type !== "text" || typeof item.text !== "string") return item;
      return {
        ...item,
...
async function refreshToken(auth, client) {
  ...
  const payload = { grant_type: "refresh_token", refresh_token: auth.refresh, client_id: CLIENT_ID };
  let refreshed = null;
  for (const endpoint of TOKEN_ENDPOINTS) {
    const res = await exchangeWithEndpoint(endpoint, payload);
    if (res.ok) { refreshed = res.json; break; }
  }
  if (!refreshed) throw new Error("Token refresh failed on all known Anthropic OAuth endpoints");
...
export async function AnthropicAuthPlugin({ client }) {
  return {
    async "experimental.chat.system.transform"(input, output) {
      ...
      const cch = Array.from(crypto.getRandomValues(new Uint8Array(3))).map(b => b.toString(16).padStart(2, "0")).join("").slice(0, 5);
      output.system.unshift(`x-anthropic-billing-header: cc_version=${VERSION}; cc_entrypoint=cli; cch=${cch};`);
    },
      ...
      async loader(getAuth, provider) {
        ...
        return {
          ...
          async fetch(input, init) {
            ...
            headers.set("anthropic-beta", [...new Set([...REQUIRED_BETAS, ...existingBetas])].join(","));
            ...
            // Transform body
            let body = typeof req.body === "string" ? transformBody(req.body) : req.body;
            ...
            if (url?.pathname === "/v1/messages") {
              if (typeof body === "string") headers.set("x-anthropic-billing-header", billingHeader(body));
              if (!url.searchParams.has("beta")) {
                url.searchParams.set("beta", "true");
                input = input instanceof Request ? new Request(url.toString(), input) : url;
              }
            }
            ...
            return {
              ...
              callback: async (code) => {
                ...
                const res = await fetch("https://api.anthropic.com/api/oauth/claude_cli/create_api_key", {
                  method: "POST",
                  headers: authHeaders({ authorization: `Bearer ${credentials.access}` }),
                });
